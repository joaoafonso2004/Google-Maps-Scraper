...
import { instagramProfileLink, isPortugueseMobile, normalizePortugalPhone, phoneLink, whatsappWebLink } from "../lib/contact-links.ts";
...
test("cria ligações de telefone e WhatsApp Web", () => {
  ...
  assert.equal(isPortugueseMobile("+351 912 345 678"), true);
  assert.equal(isPortugueseMobile("+351 210 123 456"), false);
});
