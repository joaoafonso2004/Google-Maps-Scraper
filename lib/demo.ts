...
function demoLead(input: Partial<Lead> & Pick<Lead, "id" | "name" | "reviewCount">, category: CategoryKey): Lead {
  return {
    ...
    signals: input.signals ?? {
      ...
      noApp: { status: "probable", label: "Sem app própria", detail: "Não foram encontradas ligações para App Store ou Google Play nas páginas públicas analisadas.", sourceUrl: "https://example.com" },
      manualContact: { status: "probable", label: "Contacto/processo manual", detail: "Os contactos são por telefone, email e Instagram; não foi encontrada marcação online nem portal de cliente.", sourceUrl: "https://example.com/contactos" },
...
export function getDemoLeads(category: CategoryKey, area: string, filters: SearchFilters): Lead[] {
  ...
  const leads = [
    ...
    demoLead({ id: "demo-2", name: `${label} do Jardim`, reviewCount: 67, area, address: `Avenida da República, ${area}`, signals: {
      ...
      noApp: { status: "probable", label: "Sem app própria", detail: "Nenhuma aplicação própria foi encontrada nas páginas públicas analisadas.", sourceUrl: "https://example.com" },
      manualContact: { status: "confirmed", label: "Contacto/processo manual", detail: "O telefone é o único contacto publicado e não existe marcação online ou portal de cliente.", sourceUrl: "https://example.com/contactos" },
    ...
    demoLead({ id: "demo-4", name: `${label} Norte`, reviewCount: 43, area, address: `Rua do Mercado, ${area}`, signals: {
      ...
      noApp: { status: "unverified", label: "Sem app própria", detail: "Não há páginas suficientes para avaliar este sinal." },
      manualContact: { status: "probable", label: "Contacto/processo manual", detail: "Só foi encontrado atendimento telefónico, sem marcação online identificada." },
